  ...
  const float & tolerance)
{
  ...
  while (iterations < getMaxIterations() && !_queue.empty()) {
    ...
    if (isGoal(current_node)) {
      RCLCPP_INFO_STREAM(
        ...
        "Goal found in " << iterations << " iterations and " <<
          analytic_iterations << " analytic iterations.");
      ...
      if (approach_iterations >= getOnApproachMaxIterations()) {
        RCLCPP_INFO_STREAM(
          rclcpp::get_logger("AStarAlgorithm"),
          "Approach max iterations reached, returning path to closest node to goal found in " <<
            iterations << " iterations and " << approach_iterations << " approach iterations and " << analytic_iterations << " analytic iterations.");
  ...
  if (_best_heuristic_node.first < getToleranceHeuristic()) {
    RCLCPP_INFO_STREAM(
      rclcpp::get_logger("AStarAlgorithm"),
      "Failed to find goal, returning path to closest node to goal found in " << iterations <<
        " iterations and " << analytic_iterations << " analytic iterations.");
