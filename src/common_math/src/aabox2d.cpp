...
bool AABox2d::IsPointIn(const Point &point) const {
  return std::abs(point.get<0>() - center_.x()) <= half_length_ + kMathEpsilon &&
         std::abs(point.get<1>() - center_.y()) <= half_width_ + kMathEpsilon;
}
